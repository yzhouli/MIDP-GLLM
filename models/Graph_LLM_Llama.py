...
class Attention(LlamaAttention):
...
class Block(LlamaDecoderLayer):
    def __init__(self, config, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = LlamaAttention(config=config, layer_idx=layer_idx)


class GraphLLM_Llama(LlamaModel):
    def __init__(self, config, graph_layer_num):
        super().__init__(config)
        layer_num = config.num_hidden_layers - graph_layer_num
        blocks = [LlamaDecoderLayer(config, layer_idx=i) for i in range(layer_num)]
