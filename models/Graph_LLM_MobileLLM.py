...
from transformers.models.llama4.modeling_llama4 import Llama4TextModel, Llama4TextDecoderLayer, Llama4TextAttention, \
    apply_rotary_emb, eager_attention_forward
...
class Attention(Llama4TextAttention):
...
class Block(Llama4TextDecoderLayer):
    def __init__(self, config, layer_idx):
        super().__init__(config, layer_idx)
        self.self_attn = Attention(config, layer_idx)

class GraphLLM_MobileLLM(Llama4TextModel):

    def __init__(self, config, graph_layer_num):
        super().__init__(config)
        layer_num = config.num_hidden_layers - graph_layer_num
        blocks = [Llama4TextDecoderLayer(config, layer_idx=i) for i in range(layer_num)]
